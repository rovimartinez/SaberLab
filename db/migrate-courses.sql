...
-- Policies
DROP POLICY IF EXISTS "Anyone can read cursos" ON cursos;
CREATE POLICY "Anyone can read cursos" ON cursos FOR SELECT USING (true);

DROP POLICY IF EXISTS "Admin can manage cursos" ON cursos;
CREATE POLICY "Admin can manage cursos" ON cursos FOR ALL
USING (EXISTS (SELECT 1 FROM perfiles WHERE id = auth.uid() AND role = 'admin'));

-- Insertar los 6 cursos actuales
