...
import { generateParallelSolution } from '@/lib/calculations';
import type { SolutionFilter } from '@/lib/types';
import { ParallelCircuitIcon } from './CircuitIcons';

const MIN_RESISTORS = 2;
const MAX_RESISTORS = 5;

export function ParallelCircuitTab() {
  ...
  const {
    ...
    const inverseSum = rValues.reduce((sum, r) => sum + (1 / r), 0);
    const req = inverseSum > 0 ? 1 / inverseSum : 0;
    ...
    return {
      ...
      totalCurrent: i,
      totalPower: p,
      solution: generateParallelSolution(v, rValues, filter),
    };
  ...
  return (
    <Card className="mt-4">
      <CardHeader>
        <CardTitle>Circuito en Paralelo</CardTitle>
        <CardDescription>Componentes en ramas separadas. El voltaje es constante.</CardDescription>
      </CardHeader>
                  ...
                  <CardContent className="space-y-4">
                     <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                        <div className="space-y-2 md:col-span-1">
                            <Label htmlFor="parallel-voltage">Voltaje Total (V)</Label>
                            <Input id="parallel-voltage" type="number" value={voltage} onChange={(e) => setVoltage(e.target.value)} placeholder="e.g., 12" />
                        </div>
                        {resistances.map((r, index) => (
                        <div key={index} className="space-y-2">
                            <Label htmlFor={`parallel-r${index + 1}`}>R{index + 1} (Ω)</Label>
                            <Input id={`parallel-r${index + 1}`} type="number" value={r} onChange={(e) => handleResistanceChange(index, e.target.value)} placeholder="e.g., 100" />
                        </div>
                        ))}
                    </div>
        ...
        <Card>
            <CardHeader>
             ...
             <CardContent className="p-4 flex items-center justify-center">
                <ParallelCircuitIcon numResistors={resistances.length} className="w-full h-full max-w-lg text-foreground" />
            </CardContent>
        </Card>
